...
const GENDERS = [{label: 'Dişi'}, {label: 'Erkek'}];
...
export default function AddPetScreen({navigation}: any) {
  ...
  const goNextFromStepOne = () => {
    if (!name.trim()) {
      return Alert.alert('Eksik bilgi', 'Dostunun adını gir.');
    }

    if (!type) {
      return Alert.alert('Eksik bilgi', 'Bir tür seç.');
    }
    ...
    if (type === 'Diğer') {
      ...
      if (!VALID_ANIMAL_TYPES.includes(enteredType)) {
        return Alert.alert(
          'Geçersiz tür',
          'Lütfen geçerli bir hayvan türü gir. Örneğin: Tavşan, Hamster veya Balık.',
        );
      }
    }
  ...
  const goNextFromStepTwo = () => {
    ...
    if (!gender) {
      return Alert.alert('Eksik bilgi', 'Cinsiyet seç.');
    }
  ...
  const handleSave = async () => {
    ...
    if (!currentUser) {
      return Alert.alert('Hata', 'Kullanıcı oturumu bulunamadı.');
    }

    try {
      const finalType =
        type === 'Diğer' ? customType.trim() : type;
    ...
    <LinearGradient
      ...
      <View style={styles.timelineTrack}>
        ...
        <View
          style={[
            styles.timelineDot,
            step >= 2 && styles.timelineDotActive,
          ]}>
          <View
            style={[
              styles.timelineDotInner,
        ...
        <View
          style={[
            styles.timelineDot,
            step >= 3 && styles.timelineDotActive,
          ]}>
          <View
            style={[
              styles.timelineDotInner,
      ...
      <ScrollView
        ...
        {/* ================================================= */}
        {/* DEKORLAR SCROLLVIEW İÇİNDE                       */}
        {/* Form ile birlikte yukarı kayar ve ekrandan çıkar */}
        {/* ================================================= */}
                  ...
                  <Pressable
                    ...
                    <LinearGradient
                      ...
                      {selected ? (
                        ...
                        <View
                          style={styles.checkBadgeUnselected}
                        />
                      )}
                    </LinearGradient>
                  </Pressable>
...
const styles = StyleSheet.create({
  ...
  /*
   * Dekorlar ScrollView içindedir.
   * Form ile beraber kayar ve ekranın dışına çıkar.
   */
  scrollDecoration: {
    position: 'absolute',
    zIndex: 0,
  ...
  /*
   * FORM VE DEKORLAR BİRLİKTE KAYAR
   */
  scrollView: {
    flex: 1,
    zIndex: 1,
